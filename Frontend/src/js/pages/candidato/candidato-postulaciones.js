(function () {
  ...
  async function fetchMyApplications(email) {
    try {
      ...
      const apps = res.data.map((a) => ({
        ...
        company:
          a.jobOffer?.company?.user?.fullName ||
          a.jobOffer?.company?.email ||
          "Empresa",
  ...
  function getApplicationsForEmail(email) {
    ...
    return getAllApplications().filter(
      (a) => String(a?.email || "").toLowerCase() === normalized,
    );
  }
  ...
  function renderOffers(email, isAllowed) {
    ...
    listEl
      .querySelectorAll('button[data-company-id][data-action="view-company"]')
      .forEach((btn) => {
    ...
    listEl
      .querySelectorAll('button[data-offer-id][data-action="apply"]')
      .forEach((btn) => {
        btn.addEventListener("click", function () {
  ...
  function renderApplications(email) {
    ...
    if (!sorted.length) {
      listEl.innerHTML = emptyStateHtml(
        "Aún no te postulaste",
        'Elegí una oferta y presioná "Postularme".',
      );
      return;
    }
    ...
    listEl
      .querySelectorAll('button[data-offer-id][data-action="withdraw"]')
      .forEach((btn) => {
        btn.addEventListener("click", function () {
          const offerId = btn.getAttribute("data-offer-id");
  ...
  async function init() {
    ...
    const isAllowed = Boolean(
      currentUser && currentUser.email && currentUser.role === "candidato",
    );
      ...
      axios
        ...
        .then((res) => {
          const fresh = res.data.map((job) => ({
            ...
            title: job.title,
            company:
              job.company?.user?.fullName || job.company?.email || "Empresa",
